...
class Envolvente_Compleja_PM(gr.top_block, Qt.QWidget):

    def __init__(self, Ac=100, Kp=1):
        gr.top_block.__init__(self, "Envolvente_Compleja_PM", catch_exceptions=True)
        Qt.QWidget.__init__(self)
        self.setWindowTitle("Envolvente_Compleja_PM")
        ...
        self.settings = Qt.QSettings("GNU Radio", "Envolvente_Compleja_PM")
...
def argument_parser():
    ...
    parser.add_argument(
        "--Ac", dest="Ac", type=eng_float, default=eng_notation.num_to_str(float(100)),
        help="Set Amplitud Portadora [default=%(default)r]")
    parser.add_argument(
        "--Kp", dest="Kp", type=eng_float, default=eng_notation.num_to_str(float(1)),
        help="Set Coeficiente Modulacion PM [default=%(default)r]")
    return parser


def main(top_block_cls=Envolvente_Compleja_PM, options=None):
    if options is None:
